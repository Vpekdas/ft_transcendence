...
void main() {
    ...
    if(vWorldPosition.y > t2) {
        ...
        vec3 baseColor = vec3(0.169, 0.169, 0.169) * noise;
        ...
        float noise = fbm(vWorldPosition, rep, scale, detail, roughness);
        vec3 baseColor = vec3(0.502, 0.286, 0.200);
        vec3 noiseColor = vec3(0.4, 0.2, 0.15) * noise;

        finalColor = mix(baseColor, noiseColor, 0.8);
    }

    gl_FragColor = vec4(finalColor * emissiveColor, u_opacity);
}