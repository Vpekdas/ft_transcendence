...
function generateRegistrationCode(settings, rootDirectory) {
    let components = importAllComponents("/app/" + rootDirectory);
...
let indexHtml = readFileSync("src/index.html").toString();

let s = "";
s += "<script type='module' src='" + settings.index.substring(1) + "' ></script>";
s += "<div id='micro-app'></div>";

indexHtml = indexHtml.replace("%app%", s);

writeFileSync("src/index.html", indexHtml);
